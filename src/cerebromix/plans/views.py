...
from cerebromix.plans.importer import import_name
from cerebromix.plans.mixins import PlanTableMixin, LoginRequired
from cerebromix.plans.models import UserPlan, PlanPricing, Plan, Order, BillingInfo
from cerebromix.plans.forms import CreateOrderForm, BillingInfoForm, FakePaymentsForm
from cerebromix.plans.models import Quota, Invoice
from cerebromix.plans.signals import order_started
...
# This can be very easily printed as an HTML table element with quotas by
# row.
...
class PlanTableViewBase(PlanTableMixin, ListView):
    ...
    def get_queryset(self):
        queryset = super(
            PlanTableViewBase, self).get_queryset().prefetch_related('planpricing_set__pricing',
                                                                     'planquota_set__quota')
        if self.request.user.is_authenticated():
            queryset = queryset.filter(
                Q(available=True, visible=True) & (
                    Q(customized=self.request.user) | Q(
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated():
            try:
                self.userplan = UserPlan.objects.select_related(
                    'plan').get(user=self.request.user)
...
class CurrentPlanView(LoginRequired, PlanTableViewBase):
    ...
    def get_queryset(self):
        return Plan.objects.filter(
            userplan__user=self.request.user).prefetch_related('planpricing_set__pricing',
                                                               'planquota_set__quota')
    ...
    * within current change plan policy this does not require any additional payment (None)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.user.userplan.plan != plan:
            policy = import_name(
                getattr(settings, 'PLAN_CHANGE_POLICY', 'cerebromix.plans.plan_change.StandardPlanChangePolicy'))()

            period = request.user.userplan.days_left()
            price = policy.get_change_price(
                request.user.userplan.plan, plan, period)

            if price is None:
                request.user.userplan.extend_account(plan, None)
                messages.success(
                    request, _("Your plan has been successfully changed"))
            else:
...
class CreateOrderView(LoginRequired, CreateView):
    ...
    def recalculate(self, amount, billing_info):
        ...
        # not change, tax rate is cached for a given billing data (as it mainly
        # depends on it)
    ...
    def get_currency(self):
        ...
        if len(CURRENCY) != 3:
            raise ImproperlyConfigured(
                'CURRENCY should be configured as 3-letter currency code.')
        return CURRENCY
...
class CreateOrderPlanChangeView(CreateOrderView):
    ...
    def get_policy(self):
        policy_class = getattr(settings, 'PLAN_CHANGE_POLICY',
                               'cerebromix.plans.plan_change.StandardPlanChangePolicy')
...
class OrderListView(LoginRequired, ListView):
    ...
    def get_queryset(self):
        return super(
            OrderListView, self).get_queryset().filter(user=self.request.user).select_related('plan',
                                                                                              'pricing', )
