...
from cerebromix.plans.contrib import send_template_email, get_user_language
from cerebromix.plans.enum import Enumeration
from cerebromix.plans.signals import order_completed, account_activated, account_expired, account_change_plan, account_deactivated
from cerebromix.plans.validators import plan_validation
from cerebromix.plans.taxation.eu import EUTaxationPolicy
from cerebromix.plans.managers import PlanPricingManager, PlanQuotaManager, InvoiceManager, InvoiceProformaManager, InvoiceDuplicateManager
...
class Plan(OrderedModel):
    ...
    visible = models.BooleanField(
        _('visible'), default=True, db_index=True, help_text=_('Is visible in current offer'))
    created = models.DateTimeField(_('created'), db_index=True)
    customized = models.ForeignKey(
        'auth.User', null=True, blank=True, verbose_name=_('customized'))
    quotas = models.ManyToManyField(
        'Quota', through='PlanQuota', verbose_name=_('quotas'))
...
class UserPlan(models.Model):
    ...
    def extend_account(self, plan, pricing):
        ...
        if pricing is None:
            ...
            send_template_email(
                [self.user.email], 'mail/change_plan_title.txt', 'mail/change_plan_body.txt',
            ...
            if self.plan == plan:
                ...
                # but just in case we consider a case when user has a different
                # plan
                if self.expire is None:
                    status = True
                elif self.expire > date.today():
            ...
            if status:
                ...
                mail_context = Context(
                    {'user': self.user, 'userplan': self, 'plan': plan, 'pricing': pricing})
                send_template_email(
                    [self.user.email], 'mail/extend_account_title.txt', 'mail/extend_account_body.txt',
                    mail_context, get_user_language(self.user))
    ...
    def expire_account(self):
        ...
        send_template_email(
            [self.user.email], 'mail/expired_account_title.txt', 'mail/expired_account_body.txt',
            mail_context, get_user_language(self.user))
    ...
    def remind_expire_soon(self):
        ...
        mail_context = Context(
            {'user': self.user, 'userplan': self, 'days': self.days_left()})
        send_template_email(
            [self.user.email], 'mail/remind_expire_title.txt', 'mail/remind_expire_body.txt',
            mail_context, get_user_language(self.user))
...
# return super(PlanPricingManager,
# self).get_query_set().select_related('plan', 'pricing')
...
#     def get_query_set(self):
# return super(PlanQuotaManager,
# self).get_query_set().select_related('plan', 'quota')
...
class Order(models.Model):
    ...
    plan = models.ForeignKey(
        'Plan', verbose_name=_('plan'), related_name="plan_order")
    ...
    tax = models.DecimalField(
        _('tax'), max_digits=4, decimal_places=2, db_index=True, null=True,
...
#     def get_query_set(self):
# return super(InvoiceManager,
# self).get_query_set().filter(type=Invoice.INVOICE_TYPES['INVOICE'])


# class InvoiceProformaManager(models.Manager):
#     def get_query_set(self):
# return super(InvoiceProformaManager,
# self).get_query_set().filter(type=Invoice.INVOICE_TYPES['PROFORMA'])


# class InvoiceDuplicateManager(models.Manager):
#     def get_query_set(self):
# return super(InvoiceDuplicateManager,
# self).get_query_set().filter(type=Invoice.INVOICE_TYPES['DUPLICATE'])
...
class Invoice(models.Model):
        ...
        if self.number is None:
            ...
            if invoice_counter_reset == Invoice.NUMBERING.DAILY:
                ...
                last_number = Invoice.objects.filter(
                    issued__year=self.issued.year, issued__month=self.issued.month,
    ...
    def set_issuer_invoice_data(self):
        ...
        try:
            ...
            raise ImproperlyConfigured(
                "Please set ISSUER_DATA in order to make an invoice.")
    ...
    def copy_from_order(self, order):
        ...
        self.item_description = "%s - %s" % (Site.objects.get_current()
                                             .name, order.name)
    ...
    def send_invoice_by_email(self):
        ...
        if language_code is not None:
            translation.deactivate()
        send_template_email(
            [self.user.email], 'mail/invoice_created_title.txt', 'mail/invoice_created_body.txt',
...
import cerebromix.plans.listeners
